...
namespace LeagueBot.LCU
{
    public class ClientLCU
    {
        ...
        public static void Initialize()
        {
            ...
            if (Port == 0)
            {
                Globals.logger.Write("Unable to initialize ClientLCU.cs (unable to read Api port from process)", Logger.MessageState.ERROR_FATAL);
            }
        ...
        public static bool CreateLobby(QueueEnum queueId)
        {
            using (var request = CreateRequest())
            {
                ...
                if (response == "OK")
                {
                    Globals.logger.WriteColored("Creating Lobby", Logger.Colors[Logger.MessageState.SUCCES]);
                    return true;
                ...
                else
                {
                    ...
                    switch (message)
                    {
                        ...
                        case "INVALID_LOBBY":
                            ...
                            break;
                        default:
                            Globals.logger.Write("Unknown search match result : " + obj.message, Logger.MessageState.WARNING);
                            break;
                    }
        ...
        public static void CloseClient()
        {
            ...
            {
                Globals.logger.Write("Unable request KillUX()", Logger.MessageState.WARNING);
            }
            finally
            {
